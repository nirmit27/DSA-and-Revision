...
def find_f_score(features: list[int], target: list[str]) -> None:
    ...
    for i in range(len(group_targets)):
        group_values.append(
            [feature for feature, group in zip(features, target) if group == group_targets[i]])

    f_score, p_value = anova(group_values[0], group_values[1], group_values[2])
    print(f"\nFor STRONG association, p-value = {p_value}") if p_value <= 0.05 else print(
